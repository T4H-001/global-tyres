...
CREATE POLICY IF NOT EXISTS "cache_public_read_valid"
ON public.api_data_cache
FOR SELECT
USING (expires_at > now());

CREATE POLICY IF NOT EXISTS "cache_public_insert"
ON public.api_data_cache
FOR INSERT
WITH CHECK (true);

CREATE POLICY IF NOT EXISTS "cache_public_update"
ON public.api_data_cache
FOR UPDATE
USING (true);

-- Trigger to keep updated_at fresh using existing function
CREATE TRIGGER IF NOT EXISTS update_api_data_cache_updated_at
